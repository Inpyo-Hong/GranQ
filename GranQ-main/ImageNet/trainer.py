...
class Trainer(object):
	...
	def forward(self, images, teacher_outputs, labels=None, linear=None):
        ...
        forward propagation
        """
		if "mobilenetv2_w1" in self.settings.network:
			output = self.model(images)
		else:
			output, output_1 = self.model(images, True)

		output = output.squeeze(-1).squeeze(-1) if output.dim() > 2 else output
	...
	def test(self, epoch):
		...
		with torch.no_grad():
			for i, (images, labels) in enumerate(self.test_loader):
				...
				#resnet
				output = self.model(images)  # (batch, num_classes, 1, 1)
				output = output.squeeze(-1).squeeze(-1)  # (batch, num_classes)
