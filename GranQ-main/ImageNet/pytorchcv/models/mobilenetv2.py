...
class MobileNetV2(nn.Module):
    ...
    #     feature_map = x.view(x.size(0), -1)  # 기존 방식 (2D 변환)
    #
    #     # 🔥 문제 해결: Fully Connected Layer 전에 차원 확장 (4D → 2D 변환 방지)
    #     feature_map = feature_map[:, :, None, None]  # [batch, channels] → [batch, channels, 1, 1]
    #
    #     x = self.output(feature_map)  # Fully Connected Layer 적용
    #
    #     if out_feature:
    #         return x, feature_map  # Feature map과 최종 출력 반환
    #     return x
