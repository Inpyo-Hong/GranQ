...
class Trainer(object):
    ...
    def combined_attention(self, x):
        """
		The activation maps from spatial and channel attention are respectively broadcasted
		for element-wise multiplication
		"""
        spatial_att = self.spatial_attention(x).view(x.size(0), 1, x.size(2), x.size(3))
        channel_att = self.channel_attention(x).view(x.size(0), x.size(1), 1, 1)
